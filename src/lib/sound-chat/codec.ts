/**
 * The Sound Chat codec: the thinnest honest wrapper around the two ggwave
 * instances the locked design holds for the whole page session (master plan
 * Section 3). Promoted from the Phase 0 spike with its behaviour unchanged —
 * the spike suites in `spike/` still exercise this exact code through the
 * re-export there, and the fuzz/matrix evidence in
 * `prompts/sound-chat/SOUND_CHAT_LOG.md` was measured against it.
 *
 ...
 * this helper exists so a session can measure one transmission at a time.
 *
...
/**
 * Builds the locked two-instance, one-protocol configuration: one Tx-only and
 * one Rx-only instance held for the whole session, never re-initialised per
 * message, with Rx narrowed to `AUDIBLE_FASTEST`, both fixed-length, and
